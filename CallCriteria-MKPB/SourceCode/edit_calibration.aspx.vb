...
Public Class ER2
    ...
    Public Shared Function GetAnswerOptions(ByVal id As String) As String
        ...
        Dim answers_dt As DataTable = GetTable("select *, isnull(existing_id,0) as exist_id, isnull(id,answer_id) as theID, isnull(comment,other_answer_text) as comment_text, case when b.answer_id is not null then ' checked ' else '' end as check_val from " &
            "(select answer_comments.id, comment from answer_comments with (nolock)  join calibration_scores with (nolock)  on calibration_scores.question_result = answer_id where calibration_scores.id = " & id & ") a " &
            "full outer join (select form_c_responses.answer_id,  form_c_responses.id as existing_id, form_c_responses.other_answer_text from form_c_responses with (nolock)  join calibration_scores with (nolock)  on calibration_scores.form_id = form_c_responses.form_id and calibration_scores.question_id = form_c_responses.question_id   " &
            "where calibration_scores.id =" & id & ") b  " &
            "on a.id = b.answer_id ")
        ...
        Dim check_web_dt As DataTable = GetTable("select website, view_link from calibration_scores with (nolock)  join vwForm with (nolock)  on vwForm.f_id = calibration_scores.form_id where calibration_scores.id = " & id)
    ...
    Public Shared Function UpdateQAAnswers(ByVal id As Integer, ByVal qid As Integer, ByVal name As String, ByVal answer As String) As UpdateAnswerResponse
        ...
        Try
            UpdateTable("INSERT INTO [dbo].[form_c_score_changes]([form_id],[question_id],[changed_by],[changed_date],[approved]," &
             "[approved_by],[approved_date],[new_value]) select " & id & ",(select question_id from calibration_scores where id = " & qid & "),'" & name & "',dbo.getMTDate()," &
             "1,'" & name & "',dbo.getMTDate(),(select id from question_answers where question_id = (select question_id from calibration_scores where id = " & qid & ") and answer_text = '" & answer & "')")

            Dim Sql As String = "insert into system_comments (comment_who, comment_date, comment, comment_id, comment_type) select '" & name & "', dbo.getMTDate(), '<strong>' + (select q_short_name from questions where id = (select question_id from calibration_scores where id = " & qid & "))   +  '</strong> changed to <strong>' + '" & answer & "</strong>'," & id & ", 'Calibration'"
            'Error when executing this query
            UpdateTable(Sql)

            Dim ans_dt As DataTable = GetTable("select * from calibration_scores where id = " & qid)
            If ans_dt.Rows.Count > 0 Then
                UpdateTable("update calibration_scores set question_result =  (select id from question_answers where question_id = (select question_id from calibration_scores where id = " & qid & ") and answer_text = '" & answer & "') where id = " & qid)
                UpdateTable("exec updateCalibration " & ans_dt.Rows(0).Item("form_id"))
            Else
                UpdateTable("INSERT INTO [dbo].[calibration_scores]([form_id],[question_id],question_result) select " & id & ",(select question_id from calibration_scores where id = " & qid & "),(select id from question_answers where question_id = (select question_id from calibration_scores where id = " & qid & ") and answer_text = '" & answer & "')")
            End If
            ...
            uar.success = True
            Dim rr_dt As DataTable = GetTable("select convert(decimal(10,1), total_score) as total_score,(select q_short_name from questions where id = (select question_id from calibration_scores where id = " & qid & ")) as q_changed, (select right_answer from question_answers where question_id = (select question_id from calibration_scores where id = " & qid & ") and answer_text = '" & answer & "') as right_wrong from calibration_form where id = " & id)
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Else
            'hlEdit.NavigateUrl = "/edit_record.aspx?ID=" & Request("ID")
        End If
        Dim recal_clause As String = ""

        If Request("isrecal") IsNot Nothing Then
            recal_clause = " and isrecal = '" & Request("isrecal") & "' "

            isRecal.Value = Request("isRecal")

        End If

        'already is in calibration, don't show button
        Dim cal_dt As DataTable = GetTable("select count(*) from calibration_pending where form_id =  (select top 1 id from calibration_form where original_form = " & Request("ID") & recal_clause & ")")
        ...
        Dim noti_dt As DataTable = GetTable("select count(*) from form_notifications where date_closed is null and  form_id =  (select top 1 id from calibration_form where original_form = " & Request("ID") & recal_clause & ")")
        ...
        If Not IsPostBack Then
            ...
            If Request("ID") IsNot Nothing Then
                ...
                Dim record_dt As DataTable = GetTable("select * from calibration_form join vwForm on vwForm.f_id=calibration_form.original_form  where calibration_form.id = (select top 1 id from calibration_form where original_form = " & Request("ID") & recal_clause & ")")
    ...
    Private Sub rptSections_ItemDataBound(sender As Object, e As RepeaterItemEventArgs) Handles rptSections.ItemDataBound
        ...
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            If Request("isrecal") IsNot Nothing Then
                Dim dsQuestions As SqlDataSource = e.Item.FindControl("dsQuestions")
                dsQuestions.SelectParameters("isrecal").DefaultValue = "1"
            End If
        End If
