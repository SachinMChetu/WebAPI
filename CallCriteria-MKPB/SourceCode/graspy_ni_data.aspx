<%@ Page Title="" Language="VB" MasterPageFile="~/CC_Master.master" AutoEventWireup="false" CodeFile="graspy_ni_data.aspx.vb" Inherits="graspy_ni_data" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <section class="main-container dash-modules general-button">
        ...
        <table>
            <tr>
                <td style="width: 20%; vertical-align: top">
                    ...
                    <asp:GridView CssClass="detailsTable" ID="gvAgents" AllowSorting="true" DataSourceID="dsAgents" runat="server" AutoGenerateColumns="false">
                        <Columns>
                            <asp:HyperLinkField DataNavigateUrlFields="agent, date_start, date_end" DataNavigateUrlFormatString="ch2.aspx?agent={0}&scorecard=196&start={1}&end={2}" Target="_blank" DataTextField="agent" SortExpression="Agent" HeaderText="Agent" />
            ...
            select count(*) as csa1_total,agent, sum(case when question_answered = 10102 then 1 else 0 end) as csa1_yes from vwForm join form_q_scores on form_id = f_id where question_id in (4114)
            and f_id in (select form_id from form_q_scores where question_answered in (10100)) and call_date between @start and @end
                group by agent) a
                join 
                (select count(*) as csa2_total,agent, sum(case when question_answered = 10109 then 1 else 0 end) as csa2_yes from vwForm join form_q_scores on form_id = f_id where question_id in (4117)
            and f_id in (select form_id from form_q_scores where question_answered in (10107)) and call_date between @start and @end
                group by agent) b
                on a.agent = b.agent"
                        runat="server">
                        <SelectParameters>
    ...
    <script>
        $(document).ready(function () {
            ...
            $.ajax({
                ...
                data: '{"start_date":"' + $('#ContentPlaceHolder1_txtFrom').val() + '","end_date":"' + $('#ContentPlaceHolder1_txtTo').val() + '","scorecard":"196"}', //Data sent to server
