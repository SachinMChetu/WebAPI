<%@ Page Title="" Language="VB" MasterPageFile="~/CC_Master.master" AutoEventWireup="false" ValidateRequest="false" CodeFile="edit_calibration.aspx.vb" Inherits="ER2" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function show_options(answer_id) {
            $.ajax({
                type: "POST",
                url: "edit_calibration.aspx/GetAnswerOptions",
                data: '{"id": "' + answer_id + '" }',
                //data: { id: id}, 
                //data: '{id: "' + id + '"}',
    ...
    <script type="text/javascript">
        function process(ddl, qid, name, formid) {
            ...
            $.ajax({
                type: "POST",
                url: "edit_calibration.aspx/UpdateQAAnswers",
                data: '{id: "' + id + '",qid: "' + qid + '",name: "' + name + '",answer: "' + answer + '" }',
                //data: { id: id},
                //data: '{id: "' + id + '"}',
                contentType: "application/json; charset=utf-8",
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <section class="main-container dash-modules general-button">
        ...
        <asp:HiddenField ID="isRecal" runat="server" Value="0" />
        <!-- Start page content -->
            ...
            <div class="call-table">
                ...
                <asp:Repeater ID="rptSections" DataSourceID="dsSections" runat="server">
                    ...
                    <ItemTemplate>
                        ...
                        <tr class="section-header" unselectable="on" data-spaceheight="220" style="-webkit-user-select: none;">
                            <td class="open-arrow"></td>
                        ...
                        <asp:Repeater ID="Repeater2" runat="server" DataSourceID="dsQuestions" OnItemDataBound="gvQuestions_RowDataBound">
                            <ItemTemplate>
                                <tr class="data-row <%#Eval("bad-response")%>">
                                    <td class="emptyBox">
                        ...
                        <asp:SqlDataSource ID="dsQuestions" ConnectionString="<%$ ConnectionStrings:estomes2ConnectionString %>"
                            SelectCommand="                                 
                            select fs.id as fs_id, fs.form_id, isnull(comment,form_c_responses.other_answer_text) as comment, [dbo].getTemplateText(@form_id , q.ID, 1) as template_text,
                            *, case when right_answer = 1  then 'Right' else 'Wrong' end as real_result, 
                            case when right_answer = 0 then 'bad-response' else '' end as [bad-response]
                            from dbo.calibration_scores fs
                            join [Questions] q on fs.question_ID = q.ID
                            left join question_answers qa on qa.ID = fs.question_result
                            ...
                            where fs.form_id =  (select top 1 id from calibration_form where original_form = @form_id and isrecal = @isrecal)  
                            and s.id = @section  and template not in ('Preferences')
                            order by section_order,  q_order"
                            runat="server">
                            <SelectParameters>
                                <asp:ControlParameter ControlID="hdnSectionID" Name="Section" />
                                <asp:ControlParameter ControlID="isRecal" Name="isRecal" DefaultValue="0" />
                                <asp:ControlParameter ControlID="hdnThisID" Name="form_ID" />
                ...
                <div style="text-align: left;">
                    ...
                    <asp:SqlDataSource ID="dsFormData" ConnectionString="<%$ ConnectionStrings:estomes2ConnectionString %>"
                        SelectCommand="SELECT * from  calibration_form join vwForm on vwForm.f_id=calibration_form.original_form join app_settings on app_settings.appname = vwForm.appname where vwform.f_id = @ID and isRecal = @isrecal"
                        runat="server">
                        <SelectParameters>
                            <asp:ControlParameter ControlID="hdnThisID" Name="ID" />
                            <asp:ControlParameter ControlID="isrecal" Name="isrecal" />
                        </SelectParameters>
                    </asp:SqlDataSource>
                </div>
            </div>
