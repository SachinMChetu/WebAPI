<%@ Page Title="" Language="VB" MasterPageFile="~/CC_Master.master" AutoEventWireup="false" ValidateRequest="false" CodeFile="listen_train2.aspx.vb" Inherits="listen3" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <link rel="stylesheet" href="/listen/css/listen2.css" type="text/css" />
    <script type="text/javascript" src="/listen/scripts/listen2.js?1002"></script>
    ...
    <section class="main-container">
        ...
        <div id="listenPage">
                ...
                <ItemTemplate>
                    ...
                    <asp:Repeater ID="DataList1" DataSourceID="dsQuestions"
                        ...
                        <ItemTemplate>
                            ...
                            <div class="pin-box  <%#Eval("has_options")%>" title='<%# Eval("question")%>' data-pb-qid='<%# Eval("ID")%>'>

                                   <div class="qa-float">
                                    <asp:Repeater ID="rptFAQs" runat="server" DataSourceID="dsFAQs">
                                        <ItemTemplate>
                                            <div class="faq_q"><%#Eval("question_text") %></div>
                                            <div class="faq_a"><%#Eval("question_answer")%><br />
                                                <br />
                                            </div>
                                        </ItemTemplate>
                                    </asp:Repeater>
                                ...
                                <div class="circle circle1 <%#Eval("circleYes")%>">
                                ...
                                <asp:Repeater ID="rptAnswers" runat="server" DataSourceID="dsAnswers">
                                    <ItemTemplate>
                                        <asp:HiddenField ID="hdnAnswerID" runat="server" Value='<%#Eval("ID")%>' />
                                         <div class="hidepin" data-qid="<%#Eval("question_id")%>" data-ansorder="<%#Eval("ans_order")%>" data-anstype="<%#Eval("div_class")%>">
                                            <div class="pin-time">
                                            ...
                                            <div class="pin-options-select">
                                                ...
                                                <asp:Checkboxlist ID="rblOptions" OnDataBound="rblOptions_DataBound" RepeatLayout="Flow" DataSourceID="dsResponseOptions" DataValueField="ID" DataTextField="Comment" runat="server"></asp:Checkboxlist>
                                                <%--</div>--%>
                                                <asp:SqlDataSource ID="dsResponseOptions" runat="server" ConnectionString="<%$ ConnectionStrings:estomes2ConnectionString %>"
                                                    SelectCommand="SELECT * FROM [answer_comments] where answer_id = @ANS_ID">
                                                    <SelectParameters>
                    ...
                    <asp:SqlDataSource ID="dsQuestions" ConnectionString="<%$ ConnectionStrings:estomes2ConnectionString %>" CancelSelectOnNullParameter="false"
                        SelectCommand="SELECT left(q_short_name, 20) as q_short,*, case when template != '' then 'has-sub-items' else '' end as has_options,
                                case when class is not null then 'circleYes' else '' end as circleYes,
                                case when class is not null then '0:00' else '' end as default_value
                                ...
                                where question_id in (select id from questions where scorecard_id = @scorecard_id) and autoselect = 1) b
                                on b.question_id = q.id
                                where q.section=@section and q.id in (select question_id from calibration_scores where form_id = @CalibID)
                                order by  q_order"
                        runat="server">
                        <SelectParameters>
            ...
            <asp:SqlDataSource ID="dsSections" ConnectionString="<%$ ConnectionStrings:estomes2ConnectionString %>"
                SelectCommand="select * from sections where scorecard_id = @scorecard_id order by section_order"
                runat="server">
                <SelectParameters>
                    <asp:ControlParameter ControlID="hdnScorecard" Name="scorecard_id" />
                </SelectParameters>
