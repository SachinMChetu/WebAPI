...
Partial Class create_esto_agents
    ...
    Private Sub create_esto_agents_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Dim ulist_dt As DataTable = GetTable("select username ,password  from school.school.dbo.users join (select distinct agent from xcc_report_new where agent not in (select username from userextrainfo) and appname in ('estobk','esto') and call_date > '7/25/2015') a on a.agent = username")
