<%@ Page Title="Listen" Language="VB" MasterPageFile="~/CC_Master.master" AutoEventWireup="false" ValidateRequest="false" CodeFile="listen.aspx.vb" Inherits="listen3" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <link rel="stylesheet" href="/listen/css/listen2.css?1006" type="text/css" />
    <script type="text/javascript" src="/listen/scripts/listen2.js?1011"></script>
    <script src="js/whisper2.js?1006"></script>
    ...
    <script type="text/javascript">
        function getTimeStamp(control_name) {
            ...
            //if ($('#ContentPlaceHolder1_chkTrimAudio').is(':checked'))
            //    document.getElementById("ContentPlaceHolder1_hdnTotalCallLength").value = HTML5Audio.currentTime;
            //else
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <section class="main-container">
        ...
        <asp:HiddenField ID="hdnLastClientAction" runat="server" />
        <asp:HiddenField ID="hdnFirstClientAction" runat="server" />
        <asp:HiddenField ID="hdnWhisper" runat="server" Value="0" />
        ...
        <div id="listenPage">
            ...
            <table class="page-header">
                <tr>
                    <td class="pageName">
                        <asp:Literal ID="lblThisApp" runat="server"></asp:Literal></td>
            ...
            <div class="pin-list">
                <asp:Panel ID="dynControls" Visible="false" runat="server">
                    Score each term on a scale of 0 (Not relevant) to 3 (very relevant)<br />
                    <br />


                    <style type="text/css">
                        #listenPage .pin-list .pin-box {
                        ...
                        #listenPage .pin-list .category-pins {
                            padding-top: 0px;
                            padding-bottom: 0px;
                        }
                        ...
                        <ItemTemplate>
                            ...
                            <div class="pin-options <%#Eval("has_options")%>">
                                        ...
                                        <div class="<%#eval("show_pin") %>" data-qid="<%#Eval("question_id")%>" data-ansorder="<%#Eval("ans_order")%>" data-anstype="<%#Eval("div_class")%>" data-pinans="<%#Eval("ID")%>">
                                            ...
                                            <div class="pin-options-select">
                                                ...
                                                <asp:SqlDataSource ID="dsResponseOptions" runat="server" ConnectionString="<%$ ConnectionStrings:estomes2ConnectionString %>"
                                                    SelectCommand="SELECT * FROM [answer_comments] with (nolock) where answer_id = @ANS_ID order by isnull(ac_order,99)">
                                                    <SelectParameters>
                                                ...
                                                <asp:TextBox ID="txtOtherComment" MaxLength="3500" CssClass="offset-listen-textbox" TextMode="MultiLine" runat="server"></asp:TextBox><br />
                                ...
                                <asp:SqlDataSource ID="dsAnswers" runat="server" ConnectionString="<%$ ConnectionStrings:estomes2ConnectionString %>"
                                    SelectCommand="select *, case  when right_answer = 1 and answer_text in ('Yes','No') then 'Yes' 
                                    when answer_text not in ('Yes','No') then 'NA' else 'No' end as div_class,
                                    row_number() over (order by right_answer desc) as ans_order, 
                                    (select comments_allowed from questions where id = @qid) as comments_allowed, 
                                    case when autoselect = 1 then 'showpin' else 'hidepin' end as show_pin,
                                    case when autoselect = 1 then '0:00' else '' end as default_value 
                                    from question_answers with (nolock) where question_id = @qid order by right_answer desc">
                                    <SelectParameters>
                                    ...
                                    <FooterTemplate>
                                        <span>
                                            <span>
                                                ...
                                                <asp:TextBox ID="txtOtherList" TextMode="MultiLine" MaxLength="3500" runat="server" Style="position: relative; top: 10px;"></asp:TextBox><br />
                                                <asp:CheckBox ID="chkWebsite" runat="server" />
                                                <asp:TextBox ID="txtWebsiteLink" CssClass="offset-listen-textbox" placeholder="Website...." TextMode="MultiLine" runat="server"></asp:TextBox>
                                ...
                                <br />
                                <%-- <asp:CheckBox ID="chkCommentT" runat="server" />
                                <asp:TextBox ID="txtOtherCommentT" CssClass="offset-listen-textbox" TextMode="MultiLine" runat="server"></asp:TextBox><br />--%>
                                <asp:SqlDataSource ID="dsContactList" runat="server" ConnectionString="<%$ ConnectionStrings:estomes2ConnectionString %>"
                                    SelectCommand="select 0 as orig_id,* from [dbo].[String.Split]((select top 1 template_items from question_answers with (nolock)  
                                    join questions with (nolock)  on questions.id = question_id where question_id = @qid),'|') join questions with (nolock) on questions.id = @qid
                                    where value != ''
                                    union all
                                    select * from dbo.[GetSchoolData](@xcc_id) join questions with (nolock)  on questions.id = @qid and template = 'Schools' ">
                                    <SelectParameters>
                                        <asp:ControlParameter ControlID="hdnQID" Name="QID" DefaultValue="1" />
            ...
            <asp:SqlDataSource ID="dsSections" ConnectionString="<%$ ConnectionStrings:estomes2ConnectionString %>"
                SelectCommand="select distinct case when sectionless = 1 then 0 else sections.ID end as ID, 
                case when sectionless = 1 then '' else sections.descrip end as descrip,
                case when sectionless = 1 then 'Questions' else sections.section end as section, 
            ...
            <asp:Repeater ID="rptClerked" DataSourceID="dsCollect" runat="server">
                <HeaderTemplate>
                    <div class="category-title">
                        Enter Data
                    </div>
                    <table class="detailsTable" style="padding: 10px;">
                </HeaderTemplate>
                <ItemTemplate>
                    <tr>
                        <td>
                        ...
                        <td>
                            <asp:HiddenField ID="hdnValueID" runat="server" Value='<%#Eval("ID") %>' />
                            <asp:TextBox ID="txtTextField" runat="server"></asp:TextBox>
                            <asp:RequiredFieldValidator ID="rfvText" ControlToValidate="txtTextField" runat="server" ErrorMessage="Required"></asp:RequiredFieldValidator>
                            <asp:HiddenField ID="hdnTimestamp" runat="server" />
                        </td>
                    </tr> 
                </ItemTemplate>
                <FooterTemplate>
            ...
            <asp:SqlDataSource ID="dsCollect" ConnectionString="<%$ ConnectionStrings:estomes2ConnectionString %>"
                SelectCommand="select * from sc_inputs where scorecard = @scorecard and active = 1 order by value_order" runat="server">
                <SelectParameters>
                    <asp:ControlParameter ControlID="hdnThisScorecard" Name="scorecard" />
                </SelectParameters>
            </asp:SqlDataSource>
        ...
        <div class="listen-end-buttons">
            ...
            <div class="save-buttons">
                <span class="dispute-check">
                    <asp:CheckBox ID="chkAddDispute" runat="server" Visible="false" />

                    <%--  <asp:CheckBox ID="chkTrimAudio" runat="server" />
                    <label for="chkTrimAudio">Trim Audio to Player</label>--%>

                </span>
                <span class="done-check">
                    <asp:CheckBox ID="chkStopWorking" runat="server" />
                    <label for="chkStopWorking">I'm done Working</label>
                </span>
                <span class="check-submit">Whisper #
                    <asp:DropDownList runat="server" ID="ddlWhisper">
                    ...
                    <asp:RequiredFieldValidator ID="RequiredFieldValidator2" InitialValue="" runat="server" ErrorMessage="*" Text="Choose a whisper count" ControlToValidate="ddlWhisper"></asp:RequiredFieldValidator>
                    <asp:Button ID="btnSaveSession" runat="server" Text="Save" CssClass="gradient realsubmit" />
                    <asp:Button ID="btnSaveSessionPlus" runat="server" Visible="false" Text="Save + Calibration" CssClass="gradient realsubmit" />
                </span>
            </div>
            <div class="bad-call-buttons">
                <span class="bad-call-description">If this call is bad, select the reason, and click Bad Call.</span>

                <asp:DropDownList ID="ddlDropReason" ValidationGroup="BadCall" runat="server">
                    <asp:ListItem>(Select)</asp:ListItem>
                    <asp:ListItem>Disconnected calls prior matching</asp:ListItem>
                    <asp:ListItem>Disconnected calls, no other recording found</asp:ListItem>
                    <asp:ListItem>Call transfers</asp:ListItem>
                    <asp:ListItem>Voicemail/Disconnected</asp:ListItem>
                    <asp:ListItem>Disqualified Lead</asp:ListItem>
                    <asp:ListItem>Missing Schools</asp:ListItem>
                    <asp:ListItem>Scorecard mismatch</asp:ListItem>
                    <asp:ListItem>Incomplete audio</asp:ListItem>
                    <asp:ListItem>Incomplete meta data</asp:ListItem>
                    <asp:ListItem>Audio not playing / inaudible</asp:ListItem>
                    <asp:ListItem>Unqualified lead</asp:ListItem>
                    <asp:ListItem>Duplicate Call</asp:ListItem>

                    <%--<asp:ListItem>Other</asp:ListItem>--%>
                </asp:DropDownList>
                <asp:RequiredFieldValidator ControlToValidate="ddlDropReason" ValidationGroup="BadCall" InitialValue="(Select)" runat="server" ErrorMessage="Reason Required for a Bad Call" Text="* Reason Required " ForeColor="Red"></asp:RequiredFieldValidator>

                <asp:Label ID="lblOtherReason" Text="Other reason:" runat="server"></asp:Label>
                <asp:TextBox ID="txtOtherReason" MaxLength="500" runat="server"></asp:TextBox>
                <asp:Button ID="btnBadCall" runat="server" Text="Bad Call" CausesValidation="true" ValidationGroup="BadCall" CssClass="gradient realsubmit" />
            </div>
            <UC1:five9Sesssion ID="five9update" runat="server"></UC1:five9Sesssion>
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                $('#ContentPlaceHolder1_hdnFirstClientAction').val(start);
                ...
                $("[id*='txtTextField']").each(function () {
                    $(this).change(function () {
                        //Find next *hdnTimestamp* set to audio timestamp
                        audio = $('.audio-player audio').get(0);
                        $(this).parent().find("[id*='hdnTimestamp']").val(audio.currentTime);
                        console.log(audio.currentTime);
                    })
                });

                //$("[id*='txtTextField']").change(function(item){
                //    //Find next *hdnTimestamp* set to audio timestamp
                //    audio = $('.audio-player audio').get(0);
                //    item.parent().find("[id*='hdnTimestamp']").val(audio.currentTime);
                //    console.log(audio.currentTime);
