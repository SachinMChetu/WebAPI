...
Partial Class edit_client_user
    ...
    Protected Sub FormView1_ItemUpdating(sender As Object, e As FormViewUpdateEventArgs) Handles FormView1.ItemUpdating
        ...
        UpdateTable("delete From userapps where username = '" & hdn.Value.Replace("'", "''") & "'")

        For Each li As ListItem In cbl.Items
            If li.Selected Then
                UpdateTable("insert into userapps (username, appname, dateadded, user_scorecard) select '" & hdn.Value.Replace("'", "''") & "',(Select appname from scorecards where id = '" & li.Value & "'), dbo.getMTDate(), '" & li.Value & "'")
            End If
        Next
